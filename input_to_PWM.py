...
def callback_function(data):
    ...
    v_current = enc_callback(data).v_meas
    v_ref = data.vel
    v_dot = PID.acc_calculate(v_ref, v_current)
    ################################################################################################################################################
    # Convert the velocity into motorPWM and steering angle into servoPWM
    #newECU.motor = (v_dot - -0.5742*v_current)/0.0096
    #newECU.servo = #(data.delta - 0.6850)/-0.00043611
    #################################################################################################################################################
...
servo_pwm_offset = 1539.0
# reference speed
v_ref = .8 # reference speed is 0.5 m/s
