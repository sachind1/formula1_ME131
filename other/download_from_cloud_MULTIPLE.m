...
loc_comp_id = 1120632; %input('Type your 7 digit computer ID (read it from cloud.cfg file on your BARC)?');

exp_num_total = input('How many experiments do you want to download?');
exp_struct = struct('name', [], 'exp_data', []);

% get experiment list
for idx = 1:exp_num_total;
    ...
    sig = newsig;
    sig{20} = time;
    exp_struct(idx) = struct('name', {exp_name}, 'exp_data', {sig}); %struct_fn/structfun?
    %%
